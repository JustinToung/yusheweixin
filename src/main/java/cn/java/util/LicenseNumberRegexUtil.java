...
public class LicenseNumberRegexUtil {
	private static final Log logger = LogFactory.getLog(LicenseNumberRegexUtil.class);
	/**
	 * 营业执照注册号校验正确返回码
	 */
	...
	public static String error_Businesslicense_No = "您输入的营业执照注册号位数有误，请核对后再输!";
...
//		
//		System.out.println(isLicense_18("91320115756891069F"));
//	}
	...
	public static boolean isLicense_15(String businesslicense) {
		...
		if ("".equals(businesslicense) || " ".equals(businesslicense)) {
			logger.error(error_Businesslicense_Empty);
			return false;
		} else if (businesslicense.length() != 15) {
			logger.error(error_Businesslicense_No);
			return false;
		}
...
//		getCheckCode(ints);
		if (!businesslicense15.equals(getCheckCode(ints) + "")) {// 比较填写的营业执照注册号的校验码和计算的校验码是否一致
			logger.info(error_Businesslicense);
			return false;
		}
		return true;
	}
